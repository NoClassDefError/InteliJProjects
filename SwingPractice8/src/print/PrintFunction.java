...
public class PrintFunction {
    ...
    public void init() {
        ...
        drawArea.repaint();
        drawArea.setPreferredSize(new Dimension(SIZE, SIZE));
        ...
        drawButton.addActionListener(e ->
        {
            switch (fnChooser.getItem(fnChooser.getSelectedIndex())) {
                ...
                case "triangle(a,b,c,d,e,f)":
                    ...
                    break;
                case "a sin(bx+c)+d":
                    drawArea.addPoint(new SineFn(Double.parseDouble(inputArg_a.getText().trim()), Double.parseDouble(inputArg_b.getText().trim()), Double.parseDouble(inputArg_c.getText().trim()), Double.parseDouble(inputArg_d.getText().trim())).graphFnArray);
    ...
    class DrawCanvas extends Canvas {
        private boolean[][] pxArray = new boolean[SIZE][SIZE];
...
//                            e.printStackTrace();
//                        }
                        if (lastX != SIZE / 2 && lastY != SIZE / 2 && x != SIZE / 2 && y != SIZE / 2) {
...
//            g.drawLine(lastX, lastY, 0, 0);
    ...
    class BasicFn {
        ...
        //300*300 px array of the function'SIZE graph
        public boolean[][] graphFnArray = new boolean[SIZE][SIZE];
        ...
        public double[] pxToPoint(int x, int y) {
            ...
            point[0] = ((double) x / (SIZE / 20)) - 10;
            point[1] = 10 - ((double) y / (SIZE / 20));
            return point;
        }
        ...
        public boolean isPointOnFn(double x, double y) {
            ...
            return (Math.abs(fnExp(x) - y) <= 0.01);
        }
    ...
    class SineFn extends BasicFn {
        private double A, w, fi, b;

        @Override
        public double fnExp(double x) {
            return A * Math.sin(x * w + fi) + b;
        }

        public SineFn(double A, double w, double fi, double b) {
            this.A = A;
            this.w = w;
            this.fi = fi;
            this.b = b;
